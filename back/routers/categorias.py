...
@category_router.get('/categories', tags=['categories'], response_model=list)
def get_categories() -> list:
    db = Session()
    result = CategoryService(db).get_categories()
    return JSONResponse(status_code=200, content=jsonable_encoder(result))

@category_router.get('/categories/{cat_id}', tags=['categories'], response_model=CategorySchema)
...
@category_router.post('/categories', tags=['categories'], response_model=dict, status_code=201)
...
@category_router.put('/categories/{cat_id}', tags=['categories'], response_model=dict, status_code=200)
...
@category_router.delete('/categories/{cat_id}', tags=['categories'], response_model=dict, status_code=200)
